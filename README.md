...
Each site is deployed independently: `adil-khb/discovery-assistant` and `adil-khb/-portfolio`. Discovery Assistant serves `main` → `/docs`; the portfolio keeps its existing `main` → `/` source. The source and lockfile are included for maintainability. Build the relevant target and copy its output into `docs` for Discovery Assistant, or into the repository root for the portfolio, then commit. GitHub Pages publishes the resulting update.
